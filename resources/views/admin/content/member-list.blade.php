...
<body>
	<form action="{{ url("admin/member-list") }}" method='post' name='myform' style='display:none;'>
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			...
			<tr class="text-c">
				...
				<td><u style="cursor:pointer" class="text-primary" onclick="member_show('张三','{{ url('/member-show') }}','10001','360','400')">{{ $member->username }}</u></td>
				<td>
					@if($member->sex == 0)
						女
					@else
				...
				<td class="td-manage"><a style="text-decoration:none" onClick="member_stop(this,'10001')" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a> 
					<a title="编辑" href="javascript:;" onclick="member_edit('账户密码修改','{{ url("admin/member-list/$member->id/edit") }}','1','500','400')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>  
					<!-- <a style="text-decoration:none" class="ml-5" onClick="change_password('修改密码','{{ url('/change-password') }}','10001','600','270')" href="javascript:;" title="修改密码"><i class="Hui-iconfont">&#xe63f;</i></a>  -->
					<a title="删除" href="javascript:doDel({{ "$member->id" }})"><i class="Hui-iconfont">&#xe6e2;</i></a>
...
function doDel(id)
{
	if(confirm("确定删除吗？")){
		...
		myform.action = '{{ url("admin/member-list/$member->id") }}';
		myform.submit();
	}
}
