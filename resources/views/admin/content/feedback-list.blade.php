...
<body>
	<form action="{{ url("admin/feedback-list") }}" method='post' name='myform' style='display:none;'>
...
<div class="page-container">
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-hover table-bg table-sort">
			...
			<tbody>
				<!--<?php echo "$list";?>-->
				@foreach($list as $evlue)
				<tr class="text-c">
					<td><input type="checkbox" value="1" name=""></td>
					<td>{{ $evlue->id }}</td>
					<td>
						<a href="javascript:;" onclick="member_show('{{ $evlue->uid }}','{{ url('admin/member-show')}}','10001','360','400')">
					...
					<td class="text-l"><div class="c-999 f-12">
							<u style="cursor:pointer" class="text-primary" onclick="member_show('{{ $evlue->uid }}','{{ url('admin/member-show')}}','10001','360','400')">张三</u> <time title="2014年8月31日 下午3:20" datetime="2014-08-31T03:54:20">2014-8-31 15:20</time> <span class="ml-20">13000000000</span> <span class="ml-20">admin@mail.com</span></div>
							{{ $evlue->content }}
							<!-- <div class="f-12 c-999">
								<a href="http://www.h-ui.net/Hui-4.22-comment.shtml" target="_blank">
									http://www.h-ui.net/Hui-4.22-comment.shtml
								</a>
							</div>
							<div>留言内容</div> -->
					</td>
					<td class="td-manage">
						...
						<a title="删除" href="javascript:doDel({{ "$evlue->id" }})"><i class="Hui-iconfont">&#xe6e2;</i></a>
...
function doDel(id)
{
	if(confirm("确定删除吗？")){
		...
		myform.action = '{{ url("admin/feedback-list/$evlue->id") }}';//更改上传地址
		myform.submit();
	}
}
